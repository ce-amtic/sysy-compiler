...
const int inf = 1e9, INF = 0x3f3f3f3f, mod = 998244353, maxn = 20005;
...
int main()
{
	...
	while (1)
	{
		...
		while (1)
		{
			...
			dp[i][j][1] = dp[i][j][1] + p[i] * (dp[i - 1][j - 1][0] + 1ll * g[i - 1][j - 1][0] * a[i] % mod) % mod;
			dp[i][j][1] = dp[i][j][1] % mod;
			dp[i][j][1] = dp[i][j][1] + p[i] * (dp[i - 1][j][1] + 1ll * g[i - 1][j][1] * a[i] % mod) % mod;
			dp[i][j][1] = dp[i][j][1] % mod;
